...
const router = new VueRouter({
    ...
    routes: [
        ...
        {
            path: '/register-teacher',     // 路径
            component: TeacherRegister    // 跳转到的组件
        },
        {
            path: '/register-admin',     // 路径
            component: AdminRegister    // 跳转到的组件
        },
        {
            path: '/home',     // 路径
...
router.beforeEach((to, from, next) => {
    ...
    if (to.path !== '/login2' && to.path !== '/register' && !isAuthenticated) {
        next({ path: '/login2' })
        Message({
            message: '请先登录！',
            type: "warning",
